-- ============================================================
-- AstraReach — MySQL 8 schema
-- Converted from db/schema.sql + db/migration.sql + db/whatsapp-migration.sql
-- (consolidated final state — run once on an empty database)
--
-- Requires MySQL 8.0.16+ (CHECK constraints, expression defaults)
--
-- Conversion notes:
--   UUID / gen_random_uuid()  -> CHAR(36) DEFAULT (UUID())
--   JSONB / TEXT[]            -> JSON (arrays stored as JSON arrays)
--   TIMESTAMPTZ               -> DATETIME(6), store UTC (no 2038 limit)
--   BIGSERIAL                 -> BIGINT AUTO_INCREMENT
--   NUMERIC                   -> DECIMAL
--   GIN / tsvector indexes    -> FULLTEXT index
--   Partial index (WHERE ...) -> plain composite index
--   Indexed TEXT columns      -> VARCHAR (MySQL cannot index bare TEXT)
-- ============================================================

CREATE DATABASE IF NOT EXISTS astrareach
...
-- Note: GIN indexes on tags/custom_fields have no direct MySQL equivalent.
-- If you filter on JSON keys, add multi-valued or generated-column indexes per query.
...
CREATE TABLE IF NOT EXISTS campaign_contacts (
  ...
  INDEX idx_cc_scheduled (status, scheduled_at)  -- was partial index WHERE status='queued'
) ENGINE=InnoDB;
...
-- ------------------------------------------------------------
-- email_tracking (latest status per campaign_contact)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- field_permissions (RBAC field visibility)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- user_data_scopes (row-level access control)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- permission_grants (per-user overrides)
-- ------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS wa_phone_numbers (
  ...
  CONSTRAINT chk_wapn_bsp     CHECK (bsp IN ('360dialog','twilio','meta_cloud')),
  CONSTRAINT chk_wapn_tier    CHECK (tier IN (1,2,3,4)),
  CONSTRAINT chk_wapn_quality CHECK (quality_score IN ('GREEN','YELLOW','RED'))
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS wa_templates (
  ...
  CONSTRAINT chk_wat_category CHECK (category IN ('MARKETING','UTILITY','AUTHENTICATION')),
  CONSTRAINT chk_wat_status   CHECK (status IN ('PENDING','APPROVED','REJECTED','PAUSED')),
  CONSTRAINT chk_wat_header   CHECK (header_type IN ('TEXT','IMAGE','VIDEO','DOCUMENT'))
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS wa_campaigns (
  ...
  CONSTRAINT chk_wac_status CHECK (status IN ('draft','scheduled','active','paused','completed','stopped','blocked'))
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS wa_campaign_contacts (
  ...
  CONSTRAINT chk_wacc_status CHECK (status IN ('pending','queued','sent','delivered','read','replied','failed','opted_out','invalid_number')),
...
CREATE TABLE IF NOT EXISTS wa_opt_ins (
  ...
  CONSTRAINT chk_waoi_status CHECK (status IN ('opted_in','opted_out')),
